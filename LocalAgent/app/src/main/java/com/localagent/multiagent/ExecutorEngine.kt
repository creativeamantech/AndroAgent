...
import com.localagent.agent.AgentStep
import com.localagent.agent.PromptBuilder
import com.localagent.agent.ResponseParser
...
class ExecutorEngine @Inject constructor(
    ...
    suspend fun run(task: String): Flow<String> = flow {
        ...
        val maxSteps = 10 // Fewer steps for subtasks
        var currentStep = 0

        while (currentStep < maxSteps) {
            currentStep++
            // emit("Step $currentStep: Reading screen...") // Less verbose for subtasks
            ...
            if (action == null) {
                emit("Failed to parse LLM response: $llmResponse")
            ...
            if (action.tool == "done") {
                emit("Subtask completed: ${action.args.optString("result")}")
                break
            }
        ...
        if (currentStep >= maxSteps) {
            emit("Subtask failed: Max steps reached.")
        }
    }
}
